...
import { getMonitor, removeMonitor, setDelivery, updateCursor, updateCursorValue } from "../utils"
...
export class WebhookMonitor<TSource extends MonitorSource = MonitorSource> {
  ...
  async retryPending(client: Client, source: Monitor<TSource>, record: MonitorRecord): Promise<number> {
    ...
    for (const pending of parsed.output) {
      ...
      if (current.cursors[cursorKey]) {
        await this.removePending(current, pending.deliveryId)
        continue
      }
      delivered += await this.deliver(client, source, current, pending.event, pending.deliveryId)
    }
    return delivered
  }

  private async addPending(monitor: MonitorRecord, deliveryId: string, event: MonitorEvent): Promise<void> {
    await updateCursorValue(monitor, PENDING_WEBHOOK_EVENTS, (cursor) => {
  ...
  private async removePending(monitor: MonitorRecord, deliveryId: string): Promise<void> {
    await updateCursorValue(monitor, PENDING_WEBHOOK_EVENTS, (cursor) => {
      const parsed = v.safeParse(PendingWebhookEventsSchema, cursor)
  ...
  async deliver(client: Client, source: Monitor<TSource>, record: MonitorRecord, event: MonitorEvent, deliveryId: string): Promise<number> {
    const fresh = await getMonitor(record.id)
    if (!fresh || !fresh.enabled) return 0
    ...
    if (fresh.cursors[cursorKey]) {
      await this.removePending(fresh, deliveryId)
      return 0
    }
    pendingDeliveries.add(reservationKey)
    try {
      await this.addPending(fresh, deliveryId, event)
      if (this.updateCursor) {
        ...
        await updateCursor(fresh, sourceCursorKey, cursor)
      }
      const result = await routeToSession(client, fresh, event)
      if (!result.ok) {
        console.error(`[sourcefed] webhook route failed for ${fresh.id}: ${result.error}`)
        return 0
      }
      await setDelivery(fresh.id, "webhook")
      await updateCursor(fresh, cursorKey, true)
      await this.removePending(fresh, deliveryId)
      if (event.terminal) await removeMonitor(fresh.id)
      return 1
    } finally {
